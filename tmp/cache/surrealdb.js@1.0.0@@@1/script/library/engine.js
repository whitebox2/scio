...
exports.HttpEngine = exports.WebsocketEngine = exports.Engine = exports.ConnectionStatus = void 0;
const zod_1 = require("zod");
const getIncrementalID_js_1 = require("./getIncrementalID.js");
const node_js_1 = __importDefault(require("./WebSocket/node.js"));
const index_js_1 = require("./cbor/index.js");
const types_js_1 = require("../types.js");
const errors_js_1 = require("../errors.js");
const versionCheck_js_1 = require("./versionCheck.js");
...
class WebsocketEngine {
    ...
    async connect(url) {
        ...
        const socket = new node_js_1.default(url.toString(), "cbor");
        const ready = new Promise((resolve, reject) => {
            ...
            socket.addEventListener("error", (e) => {
                const error = new errors_js_1.UnexpectedConnectionError("error" in e ? e.error : "An unexpected error occurred");
            ...
            socket.addEventListener("message", async ({ data }) => {
                const decoded = (0, index_js_1.decodeCbor)(data instanceof Blob
                ...
                if (typeof decoded == "object" && !Array.isArray(decoded) &&
                    ...
                    this.setStatus(ConnectionStatus.Error, new errors_js_1.UnexpectedServerResponse(decoded));
                }
            });
        });
        this.ready = ready;
    ...
    async rpc(request) {
        await this.ready;
        if (!this.socket)
            throw new errors_js_1.ConnectionUnavailable();
        ...
        this.socket.send((0, index_js_1.encodeCbor)({ id, ...request }));
        return response.then(([res]) => {
            if (res instanceof errors_js_1.EngineDisconnected)
                throw res;
            if ("result" in res) {
                switch (request.method) {
                    case "use": {
                        this.connection.namespace = zod_1.z.string().parse(request.params?.[0]);
                        this.connection.database = zod_1.z.string().parse(request.params?.[1]);
                        break;
                    }
                    case "signin":
                    case "signup": {
    ...
    handleRpcResponse({ id, ...res }) {
        if (id) {
            ...
            this.setStatus(ConnectionStatus.Error, new errors_js_1.ResponseError(res.error));
        }
        else {
            const live = types_js_1.LiveResult.safeParse(res.result);
            if (live.success) {
                const { id, action, result } = live.data;
                this.emitter.emit(`live-${id}`, [action, result], true);
            }
            else {
                this.setStatus(ConnectionStatus.Error, new errors_js_1.UnexpectedServerResponse({ id, ...res }));
            }
        }
    }
    get connected() {
...
class HttpEngine {
    ...
    async rpc(request) {
        ...
        if (request.method == "use") {
            const [namespace, database] = zod_1.z.tuple([zod_1.z.string(), zod_1.z.string()])
        ...
        if (request.method == "let") {
            const [key, value] = zod_1.z.tuple([zod_1.z.string(), zod_1.z.unknown()]).parse(request.params);
        ...
        if (request.method == "unset") {
            const [key] = zod_1.z.tuple([zod_1.z.string()]).parse(request.params);
        ...
        if (!this.connection.namespace || !this.connection.database) {
            throw new errors_js_1.MissingNamespaceDatabase();
        }
        const id = (0, getIncrementalID_js_1.getIncrementalID)();
        const raw = await fetch(`${this.connection.url}`, {
            ...
            body: (0, index_js_1.encodeCbor)({ id, ...request }),
        });
        const buffer = await raw.arrayBuffer();
        if (raw.status == 200) {
            const response = (0, index_js_1.decodeCbor)(buffer);
            ...
            throw new errors_js_1.HttpConnectionError(dec.decode(buffer), raw.status, raw.statusText, buffer);
        }
    }
    get connected() {
